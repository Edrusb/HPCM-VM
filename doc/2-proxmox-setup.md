# VM Implementation
For vBMC only there is no constraint on the VM needed configuration. But there is some specific
needs for a VM to be able to PXE-boot by mean of HPCM. This part is detailed here.

## Installing HPCM from an HPE provided ISO
The VM to create for the HPCM software should have:
- at least 2 network interfaces
    - one for the public network (the house network),
    - one for the HPC admin (aka "head" network)
- at least 100 GiB of disk per slot (if you want to play with two slots set a virtual disk of at least 250 GiB)
- 16 GiB or RAM seems OK, I use 32 GiB but have more than 16 GiB unused so far (all monitoring feature are setup, but no SU_Leaders configured)
...
The VM environment does not bring any issue relative to that task.

## Setup of a Compute Node VM

The problems arise with the compute node setup for PXE-boot ability:
...
You don't need any disk (unless you want to setup a SU_Leader, though disks can be added later once the VM has been created),
thus let's delete the preconfigured disk for now and click "next":

![Compute VM step3](../pictures/compute-vm-step5.png)

Choose as much CPU socket/core as you want (depending on what workload you want to have running on you virtual HPC cluster):

![Compute VM step3](../pictures/compute-vm-step6.png)

Choose as much RAM as you want (and have) still depending on the workload you expect to deploy in your virtual HPC cluster

![Compute VM step3](../pictures/compute-vm-step7.png)

Last set the first Network card you want, the other NIC(s) will be added later.

Obvious note: that the "head" Virtual Network should be connected to this VM (through this NIC or another we will add later) for HPCM to provide PXE boot support.
...
Last tune this hardware set as follows:
- a Virtio RNG (Random Number Generator) is **mandatory** for PXE Boot to work
- a serial port can be added and be used as the console output of the system (optional) see [Console on Serial](console-on-serial.md)
- one or more additional network interface(s). For example, a High Speed Network emulation
